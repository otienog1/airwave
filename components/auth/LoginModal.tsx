...
// Build-time constant: when unset, GoogleProvider renders no OAuth context,
// so the button (and its useGoogleLogin hook) must not mount at all.
const HAS_GOOGLE = !!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;

/** Owns the useGoogleLogin hook — only mounted when the provider exists. */
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={titles[mode]} size="md">
            ...
            <form onSubmit={mode === 'forgot' ? handleForgotSubmit : handleAuthSubmit} className="space-y-4">
                ...
                {/* Google sign-in — only when OAuth provider is configured */}
                {mode !== 'forgot' && HAS_GOOGLE && (
